...
export const CreditCard = ({ className, ...props }: CreditCardProps) => (
  <div
    className={cn(
      'group/kibo-credit-card perspective-[600px] aspect-[8560/5398] w-[100%] max-w-[24rem] text-[#ffffff]',
      '@container',
      className
    )}
    {...props}
    ...
    <CreditCardFlipContext.Provider value={true}>
      ...
      <div
        ...
        className={cn(
          ...
          'transform-3d transform-style-[preserve-3d] transition duration-[700ms] ease-in-out transform-origin-[center]',
          supportsHover &&
          'group-hover/kibo-credit-card:rotate-y-[-180deg] group-hover/kibo-credit-card:shadow-lg',
          !supportsHover && isFlipped && 'rotate-y-[-180deg] shadow-lg',
          className
        )}
        {...props}
      >
...
export const CreditCardName = ({
  ...
  <p
    className={cn('font-semibold uppercase transform scale-x-[-1]', className)}
    style={{
      lineHeight: '100%',
      ...style,
    }}
...
export const CreditCardChip = ({
  ...
  children ? (
    <div
      className={cn(
        'translate-y-[-50%] absolute top-[50%] left-[0] w-[16.666667%] shrink-[0] rounded-[18%]',
        className
      )}
    >
      {children}
...
export const CreditCardLogo = ({
  ...
  <div
    className={cn('absolute top-[0] right-[0] w-[16.666667%] h-[16.666667%]', className)}
    {...props}
  />
);
...
export const CreditCardFront = ({
  ...
  ...props
              ...
              <div
                className={cn(
                        'scale-x-[-1] border-[12px] border-[#483300FF] backface-visible absolute inset-[0] flex flex-row-reverse bg-[rgba(var(--foreground),0.9)]',
                        '@xs:rounded-2xl rounded-lg',
                        className
                )}
                {...props}
    ...
    return (
      <div
        className={cn(
          ...
          'max-height-[33.333333%] max-width-[33.333333%]',
          className
        )}
      >
        {children}
...
export const CreditCardBack = ({
  ...
  return (
    <CreditCardBackContext.Provider value={{ safeArea }}>
      <div
        className={cn(
      'border-[5px] border-[#FFC642FF]  backface-hidden absolute inset-[0] flex bg-[#F2F2F2]',
      '@xs:rounded-2xl rounded-lg',
        )}
        {...props}
      >
...
export const CreditCardMagStripe = ({
  ...
  return (
    <div
      className={cn(
        'translate-x-[-50%] absolute top-[3%] left-[50%] h-[25%] bg-[#1a202c]',
        className
      )}
      style={{
...
export type CreditCardNumberProps = HTMLAttributes<HTMLDivElement> & {
...
export const CreditCardNumber = ({
  ...
  children,
  style,
  truncateLength = 10, // Default truncate length
  ...props
}: CreditCardNumberProps) => {
  const formatAddress = (address: ReactNode, length: number) => {
    const addressString = String(address);
    if (addressString.length <= length) return addressString;
    const start = addressString.substring(0, length / 2);
    const end = addressString.substring(addressString.length - length / 2);
    return `${start}...${end}`;
  };

  console.log('Formatted Address:', formatAddress(children, truncateLength));

  return (
    <div
      className={cn('font-mono', '@xs:text-[1.5rem]', 'text-wrap', 'overflow-wrap-[break-word]', 'text-pretty', className)}
      style={{
        lineHeight: '100%',
        ...style,
      }}
      {...props}
    >
      {formatAddress(children, truncateLength)}
    </div>
  );
};
